...
function getLevel(confidence: number) {
  ...
  if (confidence < 35) {
    return {
      name: "Observation",
      description:
        "OASIS repère vos premières tendances.",
    };
  }
  ...
  if (confidence < 65) {
    return {
      name: "Compréhension",
      description:
        "Votre profil devient plus précis.",
    };
  }
  ...
  return {
    ...
    description:
      "OASIS adapte ses recommandations à votre historique.",
  };
}
  ...
  const confidence = Math.min(
    ...
    Math.round(
      (ingredient.confidence || 0) *
        0.4 +
        (ingredient.correlation_confidence ||
          0) *
          0.4 +
        Math.min(
          skinEntries * 5,
